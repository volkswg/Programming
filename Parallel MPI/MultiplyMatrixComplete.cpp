...
int main(int argc, char *argv[])
{
	...
	char *fileC = "C:\\Users\\DELL\\Desktop\\KMUTT\\CPE374 - Paralell Computing\\Homework\\MatrixSet\\MatrixSet\\Medium\\out.txt";
	...
	for (i = 0; i < nproc; i++)
	{
		...
		if (i != 0) {
			displsA[i] = displsA[i] + scountA[i - 1] + displsA[i - 1];
		}
		else {
			displsA[i] = 0;
		}
	...
	for (i = 0; i<nproc; i++)
	{
		...
		if (i != 0) {
			gDispls[i] = gDispls[i] + gsRecv[i - 1] + gDispls[i - 1];
		}
		else {
			gDispls[i] = 0;
		}
